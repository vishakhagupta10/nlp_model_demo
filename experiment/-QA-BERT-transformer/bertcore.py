...
def use_model(model_name, config_file_path, model_file_path, vocab_file_path, num_labels):
    ...
    if(model_name == 'bert'):
        ...
        model = model_class.from_pretrained(model_file_path, from_tf=bool('.ckpt' in 'bert-base-uncased'), config=config)
...
def to_bert_ids(tokenizer,q_input):
    #Convert text input into corresponding id number
...
#All answers (including duplicates)
        self.answers_norepeat = sorted(list(set(answers))) # non repeating
        self.answers_types = len(self.answers_norepeat) # how many category
        self.ans_list = [] # List to find id or text
        self._make_dic() # make a dict
...
def convert_data_to_feature(tokenizer, train_data_path):
    with open(train_data_path, "r") as reader:
    ...
    for entry in tqdm(input_data):
        for paragraph in entry["questions"]:
            list_context = paragraph["ideal_answer"][0]
            question_text = paragraph["body"]
            try:
                questions.append(question_text)
                answers.append(list_context)
            except:
                continue
    ...
    assert max_seq_len <= 512 # bert base  max limit
