...
def run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, best_dev_score):
    ...
    if args.do_train and args.do_predict:
        ...
        acc = evaluate(dataset, predict, tokenizer)['acc']
        exact_match = evaluate(dataset, predict, tokenizer)['exact_match']
        logger.info('F1 score : {}'.format(dev_score))
        #    logger.info('metric_max_over_ground_truths : {}'.format(exact_match))
        logger.info('Accuracy : {}'.format(acc))

    if best_dev_score is not None and dev_score > best_dev_score:
        logger.info('Saving model with dev score 23456789 : {}'.format(dev_score))
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--bert_config_file", default='biobert_v1.1_pubmed/bert_config.json', type=str,
                        help="The config json file corresponding to the pre-trained BERT model. "
                             "This specifies the model architecture.")
    parser.add_argument("--vocab_file", default='biobert_v1.1_pubmed/vocab.txt', type=str,
    ...
    parser.add_argument("--init_checkpoint", default='biobert_v1.1_pubmed/pytorch_model.bin', type=str,
...
#        run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, best_dev_score )
    if args.do_predict and args.do_train:
        run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, best_dev_score)
    if args.do_predict and args.do_train == False:
        run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, None)
