...
def use_model(model_name, config_file_path, model_file_path, vocab_file_path, num_labels):
    # 選擇模型並加載設定
    if(model_name == 'bert'):
        from transformers import BertConfig, BertForSequenceClassification, BertTokenizer, AutoTokenizer, AutoModel
...
#    with open(train_data_path,'r',encoding='utf-8') as f:
    ...
    for entry in tqdm(input_data):
        ...
        #    s = ""
        #    for sent in list_context:
        #        s = s + sent + " "

        #    paragraph_text = " ".join(tokenization.whitespace_tokenize(clean_text(s)))
        #    qas_id = paragraph["id"]
