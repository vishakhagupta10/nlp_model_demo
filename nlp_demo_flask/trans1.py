...
def eval(outside_sent):
  ...
  decoder = DecoderAttn(hidden_size, output_lang.vocab_size, layers=layers, 
                        dropout=dropout, bidirectional=bidirectional)
  encoder.load_state_dict(torch.load('_enc_weights.pt', map_location=torch.device('cpu')))
  decoder.load_state_dict(torch.load('_dec_weights.pt', map_location=torch.device('cpu')))
  #mem()
